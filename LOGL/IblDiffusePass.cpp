...
void IblDiffusePass::init() {
	std::cout << "generating ibl irradiance map\n";
...
void IblDiffusePass::update() {
	...
	unsigned int temp = glGetUniformLocation(irradianceShader.Program, "environmentMap");
	glUniform1i(glGetUniformLocation(irradianceShader.Program, "environmentMap"), 0);
	glUniform1i(glGetUniformLocation(irradianceShader.Program, "equirectangularMap"), 1);
